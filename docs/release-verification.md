...
CI 模式下 flaky（重试才通过）也判定失败。排查布局时序问题时，应按选中 tab 的名称定位目标面板，等待选中状态、可见性与有效尺寸，使用 `e2e/layout-assertions.ts` 的 `visibleBox` 读取尺寸；不得用非空断言掩盖隐藏或卸载中的元素。对调度敏感的修复先做无重试重复验证，必要时用 CPU 降速复现慢运行环境，再执行完整门禁。
