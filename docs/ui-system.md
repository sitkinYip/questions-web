...
- `useWorkspaceViewport` 统一测量可视视口与 tab 起点。剩余阅读高度不足 180px 时，工作区设置最小阅读高度并切换为主内容区单层滚动，顶栏仍固定；恢复高度后自动回到 tab 内滚动。监听 ResizeObserver、窗口及 visualViewport 变化，测量抵消主内容区 scrollTop，避免滚动中反复切换。桌面外壳不再强制 620px 最小高度。
- 启程底部同步提示使用主题色半透明渐变承接列表，底部预留阅读留白，装饰不拦截点击。短屏继续隐藏辅助提示。`e2e/workspace-viewport.spec.ts` 覆盖明暗主题、三种横屏宽度、启程/来信的实际滚动及旋回竖屏。
